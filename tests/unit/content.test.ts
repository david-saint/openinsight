...
describe("Content Script", () => {
  beforeEach(() => {
    ...
    // Create a mock text node for the selection's endContainer
    const mockTextNode = document.createTextNode("selected text");

    // Mock window.getSelection with proper endContainer and endOffset
    const selectionMock = {
    ...
    // Mock document.createRange for the endPosition calculation
    ...
    vi.spyOn(document, "createRange").mockImplementation(() => {
      const range = {
        ...
        getBoundingClientRect: vi
          .fn()
          .mockReturnValue({ left: 100, bottom: 20 }),
      };
  ...
  it("should log selected text on mouseup", async () => {
    ...
    expect(console.log).toHaveBeenCalledWith(
      "OpenInsight content script initialized."
    );
  });
